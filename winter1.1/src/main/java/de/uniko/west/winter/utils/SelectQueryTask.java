 ...
 *
 */
public class SelectQueryTask extends DefaultQueryTask {

	public SelectQueryTask(InfoNode node, URI graph, Map<String, URI> prefixMap, QueryTaskExecutor executor, String ... var) {
	...
	protected void insertTask() {
		insertGraph();
		VarSubstitutionVisitor vsVisitor = new VarSubstitutionVisitor(node, vars);
//		PatternReductionVisitor prVisitor = new PatternReductionVisitor();
		
		visitQueryPattern(vsVisitor);
		...
		sb.insert(0, "SELECT"+varsString+" WHERE {\n");
		sb.append("}\n");
	}
